...
class SmartCompressor:
    ...
    def generate_transform_method(self, marker):
        """
        Generate transformation methods for markers 12-255: XOR with size-based value, repeated 1000 times.
        """
        def transform(data, repeat=1000):
            ...
            transformed = bytearray(data)
            data_size = len(data)
            scale_factor = max(2000, min(256000, data_size))  # Scale from 2000 to 256000
    ...
    def compress_with_best_method(self, data, filetype, input_filename, mode="slow"):
        ...
        # Fast mode: Use transformations 1, 3, 4, 5, 6, 7, 8, 9 (from PAQJP_4)
        ...
        # Slow mode: Use all transformations (1-11, 12-255)
        ...
        # Select transformations based on mode
        transformations = slow_transformations if mode == "slow" else fast_transformations

        # Prioritize transformations 7, 8, 9, 10, 11 for JPEG/TEXT in slow mode; 7, 8, 9 in fast mode
...
def main():
    ...
    # Prompt for fast or slow mode if compressing
    mode = "slow"  # Default to slow mode
    if choice == '1':
        try:
    ...
    if choice == '1':
        ...
        compressed = compressor.compress_with_best_method(input_data, filetype, input_file, mode=mode)
        if compressed is None:
            return
        ...
        try:
            ...
            if marker in [7, 8, 9, 10, 11] + list(range(12, 256)) and not output_file.endswith(('.jpg', '.jpeg', '.txt')):
