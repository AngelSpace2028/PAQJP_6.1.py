...
PI_DIGITS_FILE = "pi_digits.txt"  # File for base-10 pi digits
...
def save_pi_digits(digits: List[int], filename: str = PI_DIGITS_FILE) -> bool:
    try:
        ...
        logging.info(f"Saved {len(digits)} base-10 pi digits to {filename}")
        return True
    ...
    try:
        ...
        with open(filename, 'r') as f:
            ...
            if len(digits) != expected_count:
                ...
                return None
            logging.info(f"Loaded {len(digits)} base-10 pi digits from {filename}")
            return digits
    except Exception as e:
...
def generate_pi_digits(num_digits: int = 3, filename: str = PI_DIGITS_FILE) -> List[int]:
    ...
    try:
        ...
        logging.warning(f"Using {len(mapped_fallback)} fallback digits")
...
class SmartCompressor:
    ...
    def transform_01(self, data, repeat=1000):
        if not data:
    ...
    def reverse_transform_01(self, data, repeat=1000):
        if not data:
    ...
    def transform_04(self, data, repeat=1000):
        if not data:
    ...
    def reverse_transform_04(self, data, repeat=1000):
        if not data:
    ...
    def transform_07(self, data, repeat=1000):
        if not data:
    ...
    def reverse_transform_07(self, data, repeat=1000):
        if not data:
    ...
    def transform_08(self, data, repeat=1000):
        if not data:
    ...
    def reverse_transform_08(self, data, repeat=1000):
        if not data:
    ...
    def transform_09(self, data, repeat=1000):
        if not data:
    ...
    def reverse_transform_09(self, data, repeat=1000):
        if not data:
    ...
    def transform_10(self, data, repeat=1000):
        if not data:
    ...
    def reverse_transform_10(self, data, repeat=1000):
        if len(data) < 1:
    ...
    def transform_11(self, data, repeat=1000):
        if not data:
    ...
    def reverse_transform_11(self, data, repeat=1000):
        if len(data) < 1:
    ...
    def generate_transform_method(self, marker):
        def transform(data, repeat=1000):
            ...
            size_mod = (data_size % scale_factor + marker) % 256
    ...
    def get_transformations_for_level(self, level: int) -> Tuple[List[Tuple[int, callable]], int]:
        """Return transformations and repeat value for the given compression level."""
        transformations = [
            (1, self.transform_04),
            (2, self.transform_01),
            (9, self.transform_09),
            (11, self.transform_11),
            (255, self.generate_transform_method(255)[0]),
            (65535, self.generate_transform_method(65535)[0]),
            (2**24 - 1, self.generate_transform_method(2**24 - 1)[0]),
        ]
        level_configs = {
            1: ([transformations[0]], 1000),  # transform_04
            2: (transformations[:2], 2000),   # transform_04, transform_01
            3: (transformations[:3], 3000),   # transform_04, transform_01, transform_09
            4: (transformations[:4], 4000),   # transform_04, transform_01, transform_09, transform_11
            5: (transformations[:5], 5000),   # transform_04, transform_01, transform_09, transform_11, transform_255
            6: (transformations, 6000),       # all transformations
        }
        return level_configs.get(level, (transformations, 6000))  # Default to level 6

    def compress_with_best_method(self, data, filetype, input_filename, level=6):
        if not data:
            logging.warning("compress_with_best_method: Empty input data, returning minimal marker")
            return struct.pack('>I', 0)[:3]  # 3-byte marker
        dt = get_file_creation_time(input_filename)
        datetime_bytes = encode_datetime(dt)
        
        transformations, repeat = self.get_transformations_for_level(level)
        
        # Prioritize transformations for JPEG and TEXT files
        if filetype in [Filetype.JPEG, Filetype.TEXT]:
            prioritized = [(9, self.transform_09), (11, self.transform_11)]
            if level >= 5:
                prioritized.append((255, self.generate_transform_method(255)[0]))
            if level == 6:
                prioritized.extend([
                    (65535, self.generate_transform_method(65535)[0]),
                    (2**24 - 1, self.generate_transform_method(2**24 - 1)[0])
                ])
            transformations = prioritized + [t for t in transformations if t[0] not in [9, 11, 255, 65535, 2**24 - 1]]
        ...
        for marker, transform in transformations:
            transformed = transform(data, repeat=repeat)
            for method_name, compress_func in methods:
                try:
                    compressed = compress_func(transformed[1:] if marker in [10, 11] else transformed)
                    if compressed is None:
                        continue
                    if marker in [10, 11]:
                        compressed = transformed[:1] + compressed  # Include n or y
                    size = len(compressed)
                    if size < best_size:
                        best_size = size
                        ...
                        n_or_y = transformed[0] if marker in [10, 11] else None
                except Exception as e:
        ...
        if best_compressed is None:
            ...
            return struct.pack('>I', 0)[:3] + datetime_bytes + data

        logging.info(f"Best compression method: {best_method}, Marker: {best_marker} for {filetype.name} at level {level}")
        return struct.pack('>I', best_marker)[:3] + datetime_bytes + best_compressed
    
    def decompress_with_best_method(self, data):
        if len(data) < 9:  # 3 bytes marker + 6 bytes datetime
            logging.warning("decompress_with_best_method: Insufficient data, returning empty bytes")
            return b'', None
        
        method_marker = int.from_bytes(data[:3], 'big')
        datetime_bytes = data[3:9]
        ...
        reverse_transforms = {
            ...
            255: self.generate_transform_method(255)[1],
            65535: self.generate_transform_method(65535)[1],
            2**24 - 1: self.generate_transform_method(2**24 - 1)[1],
        }
        ...
        try:
            decompressed = self.paq_decompress(compressed_data[1:] if method_marker in [10, 11] else compressed_data)
            if not decompressed:
                logging.warning("PAQ decompression returned empty data")
                return b'', None
            if method_marker in [10, 11]:
                decompressed = compressed_data[:1] + decompressed
            result = reverse_transforms[method_marker](decompressed, repeat=6000)  # Use max repeat for decompression
            ...
            decompressed = self.decompress_data_zlib(compressed_data[1:] if method_marker in [10, 11] else compressed_data)
...
def main():
    ...
    if choice == '1':
        try:
            level_input = input("Enter compression level (1-6): ").strip()
            level = int(level_input)
            if level not in range(1, 7):
                logging.warning("Invalid compression level. Defaulting to level 6.")
                level = 6
        except (ValueError, EOFError):
            logging.info("Invalid or no level input detected. Defaulting to level 6.")
            level = 6
        logging.info(f"Selected compression level: {level}")
    ...
    if choice == '1':
        ...
        compressed = compressor.compress_with_best_method(input_data, filetype, input_file, level=level)
        if compressed is None:
            return
        ...
        try:
            ...
            if marker in [9, 11, 255, 65535, 2**24 - 1] and not output_file.endswith(('.jpg', '.jpeg', '.txt')):
